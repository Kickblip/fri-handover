...
def to_3d(calib, u: int, v: int, depth_mm: float) -> np.ndarray:
    ...
    Returns (X, Y, Z) in meters in a camera-centered 'world' frame:

        X = camera X (left/right)
        Y = camera Z (depth, forward from camera)
        Z = -camera Y (upwards)

    So:
      - origin is at the camera center
      - +X is to the right in the image
      - +Y is farther away from the camera
      - +Z is upwards
    """
    if depth_mm <= 0:
    ...
    try:
        # Azure Kinect returns camera coordinates in mm:
        # x_mm: right, y_mm: down, z_mm: forward
    ...
    # Camera coords in meters
    x_c = x_mm / 1000.0
    y_c = y_mm / 1000.0
    z_c = z_mm / 1000.0

    # Remap to nicer world-ish frame:
    # X = X_c (left/right)
    # Y = Z_c (depth)
    # Z = -Y_c (up)
    X = x_c
...
def extract_3d_landmarks_and_overlay() -> Tuple[List[List[np.ndarray]], float]:
    ...
    converts landmarks to 3D using depth + calibration,
    ...
    Returns:
        frames_xyz: list over frames;
                    ...
                    in the camera-centered world-ish frame described above.
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            # 2) Compute 3D XYZ per hand (in remapped world-ish frame)
...
def save_csv(frames_xyz: List[List[np.ndarray]]) -> None:
    ...
    Values are in the camera-centered world-ish frame:
      X (m, left/right), Y (m, depth), Z (m, up).
    """
    rows = []
...
def compute_axis_limits(frames_xyz: List[List[np.ndarray]]):
    ...
    if not all_pts:
        # Reasonable default range in meters
        return -0.5, 0.5, 0.2, 2.0, -0.5, 1.5
...
def render_3d(frames_xyz: List[List[np.ndarray]], fps: float) -> None:
    ...
    Axes (meters) match the remapped world-ish frame:
